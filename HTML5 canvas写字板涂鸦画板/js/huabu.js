	...
	draw:function(){
		...
		$('#cavs').mousemove(function(e){//当鼠标在画布上移动时执行
			if(painting===true){//判断是否是可绘画状态
				...
				console.log('mousemove-x', x - canX );
				console.log('mousemove-x', y - canY);
				let point = {
					x: x - canX,
					y: y - canY,
				}
				if (drawLines.length > 0) {
					...
					lastLine.push(point);
					console.log('mousemove-drawLines', drawLines);
				}
		...
		$('#cavs').mousedown(function(e){//当鼠标按下时触发
			...
			console.log('x', p_x - canX);
			console.log('y', p_y - canY);

			let startPoint = {
				x: p_x - canX,
				y: p_y - canY,
			}
			drawLines.push([startPoint]);
			console.log('mousedown-drawLines', drawLines);
